...
void	eur_menu(void)
{
	...
	new_unselectable_entry("ACNL NTR Cheats Ver 3.0 Alpha");
	index = new_spoiler("Códigos Enviroment");
	set_note("Example note", index);
		new_entry("Regar todas las flores", quench_eur);
		new_entry("Sembradora", seed_eur);
		new_entry("Buscar y reemplazar", search_eur);
		new_radio_entry("Césped", grass_eur);
		new_radio_entry("Desierto", desert_eur);
		new_entry("Retirar todas las malas hierbas", weeder_eur);
		new_entry("árbol instantánea", tree_eur);
	exit_spoiler();
	new_spoiler("Jugador códigos 1");
		new_entry("Texto con el punto", text2item_usa);
		new_entry("Duplicación", duplicate_usa);
		new_entry("Máximo Bronceado", tan_usa);
	exit_spoiler();
	new_spoiler("Jugador códigos 2");
		new_entry("Texto con el punto", text2item2_usa);
		new_entry("Duplicación", duplicate2_usa);
		new_entry("Máximo Bronceado", tan2_usa);
	exit_spoiler();
	new_spoiler("Jugador códigos 3");
		new_entry("Texto con el punto", text2item3_usa);
		new_entry("Duplicación", duplicate3_usa);
		new_entry("Máximo Bronceado", tan3_usa);
	exit_spoiler();
	new_spoiler("Jugador códigos 4");
		new_entry("Texto con el punto", text2item4_usa);
		new_entry("Duplicación", duplicate4_usa);
		new_entry("Máximo Bronceado", tan4_usa);
	exit_spoiler();
	new_spoiler("Códigos de Movimiento");
		new_entry("Warping", warping_eur);
		new_entry("Luna saltar", moonjump_eur);
		new_entry("Coordina Modificador", coord_eur);
		new_entry("Velocidad", speed_eur);
		new_entry("Teleport", teleport_eur);
	exit_spoiler();
	new_spoiler("Mejoras de la Tienda");
		new_radio_entry("Merca T&N", nook1_eur);
		new_radio_entry("Súper T y N", nook2_eur);
